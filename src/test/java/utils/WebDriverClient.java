...
public class WebDriverClient {
	 ...
	 * @throws Exception
	 */
	/*public boolean waitForInvisibilityofElementLocatedBy(WebElement element, int timeOutInMilliSeconds) throws Exception {
		...
		try{
			...
			if(timePassedinSS > 0){
				Reporter.log(String.valueOf(timePassedinSS) + "(s) passed waiting for invisibility of element => " + element.getAttribute("id") , true);
			}
			return isElementNotPresent;
		}catch(Exception e){
			Reporter.log(String.valueOf((clock.now() - start) / 1000) + "(s) passed waiting for invisibility of element, then came in exception", true);
		}
		finally{
			resetImplicitWait();
		}
	...
	*//**
	 ...
	 * @throws Exception
	 *//*
	public boolean waitForInvisibilityofElementLocatedBy(String locator,int timeOutInMilliSeconds) throws Exception {
		long start = clock.now();
		long timePassedinSS;
		try{
			setImplicitWait(timeOutInMilliSeconds);
			...
			if(timePassedinSS > 0){
				Reporter.log(String.valueOf(timePassedinSS) + "(s) passed waiting for invisibility of element => " + locator , true); 
			}
			return isElementNotPresent;
		}catch(Exception e){
	...
	*//**
	 * Wait for locator of multiple objects to be invisible or -1 on the DOM
	 * @param locator used to find the element
	 * @return true if element is invisible, false otherwise
	 * @throws Exception
	 *//*
	public boolean waitForInvisibilityofOneElementLocatedBy(String locator, Integer originalNumOfElements) throws Exception {
		long start = clock.now();
		long timePassedinSS;
		try{
			boolean isElementNotPresent = webdriverWait.until(ExpectedConditions
					.numberOfElementsToBeLessThan(By.xpath(locator), originalNumOfElements)).size() == originalNumOfElements-1;
			timePassedinSS = (clock.now() - start) / 1000;
			if(timePassedinSS > 0){
				Reporter.log(String.valueOf(timePassedinSS) + "(s) passed waiting for invisibility of element => " + locator , true); 
			}
	...
	*//**
	 * Wait for text to be visible or present on the DOM
	 * @param locator used to find the element
	 * @return true if text is visible, false otherwise
	 * @throws Exception
	 *//*
	public boolean waitForVisibilityofElementTextLocatedBy(String locator, String text) throws Exception {
		long start = clock.now();
		long timePassedinSS;
		try{
			boolean isElementPresent = webdriverWait.until(ExpectedConditions.textToBePresentInElementLocated(By.xpath(locator), text));
			timePassedinSS = (clock.now() - start) / 1000;
			locator = locator.contains("@class='ui-block'")?"Spinner":locator;
			if(timePassedinSS > 0){
				Reporter.log(String.valueOf(timePassedinSS) + "(s) passed waiting for invisibility of element => " + locator , true); 
